...
export default function Pengetahuan() {
    ...
    React.useEffect(() => {
        if (localStorage.getItem('data_siswa')) {
            ...
            const nilaiSiswa = JSON.parse(localStorage.getItem('data_nilai_pengetahuan'))

            setSiswa(listSiswa.filter((s) => !nilaiSiswa || !nilaiSiswa.some((n) => n.id_siswa === s.id)))

        }
        if (localStorage.getItem('data_soal_pengetahuan')) {
            setSoal(JSON.parse(localStorage.getItem('data_soal_pengetahuan')))
        }
    }, [])
    ...
    function resetNilai() {
        localStorage.removeItem('data_nilai_pengetahuan')
        if (localStorage.getItem('data_siswa')) {
...
function Modal({ siswa, modalState, soal }) {
    ...
    function appendNilai(n) {
        ...
        if (s < soal.length - 1) {
            ...
            setNilai([...nilai, n])
            const dataNilai = JSON.parse(localStorage.getItem('data_nilai_pengetahuan')) || []
            localStorage.setItem('data_nilai_pengetahuan', JSON.stringify([
                ...dataNilai,
                {
                    id_siswa: siswa.id,
