...
export default function Keterampilan() {
    ...
    React.useEffect(() => {
        if (localStorage.getItem('data_siswa')) {
            ...
            const nilaiSiswa = JSON.parse(localStorage.getItem('data_nilai_keterampilan'))

            setSiswa(listSiswa.filter((s) => !nilaiSiswa || !nilaiSiswa.some((n) => n.id_siswa === s.id)))

        }
        if (localStorage.getItem('data_soal_keterampilan')) {
            setSoal(JSON.parse(localStorage.getItem('data_soal_keterampilan')))
        }
    }, [])
    ...
    function resetNilai() {
        localStorage.removeItem('data_nilai_keterampilan')
        if (localStorage.getItem('data_siswa')) {
...
function Modal({ siswa, modalState, soal }) {
    ...
    function appendNilai(n) {
        ...
        if (s < soal.length - 1) {
            ...
            setNilai([...nilai, n])
            const dataNilai = JSON.parse(localStorage.getItem('data_nilai_keterampilan')) || []
            localStorage.setItem('data_nilai_keterampilan', JSON.stringify([
                ...dataNilai,
                {
                    id_siswa: siswa.id,
