...
function ImportSoalModal({ modalState }) {
    ...
    function processFile(file) {
        ...
        reader.onload = () => {
            ...
            setSoalData(soal);
            modalState(false);
            localStorage.setItem('data_soal_pengetahuan', JSON.stringify(soal))
        };
...
export default function SoalPengetahuan() {
    const [importSiswa, setImportSoal] = React.useState(false);
    const [soalData, setSoalData] = useState([]);

    React.useEffect(() => {
        setSoalData(JSON.parse(localStorage.getItem('data_soal_pengetahuan')) || [])
    }, [])

    function clearSoal() {
        setSoalData([])
        localStorage.removeItem('data_soal_pengetahuan')
    }
