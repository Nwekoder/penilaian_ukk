...
function ImportSoalModal({ modalState }) {
    ...
    function processFile(file) {
        ...
        reader.onload = () => {
            ...
            setSoalData(soal);
            modalState(false);
            localStorage.setItem('data_soal_keterampilan', JSON.stringify(soal))
        };
...
export default function SoalKeterampilan() {
    const [importSiswa, setImportSoal] = React.useState(false);
    const [soalData, setSoalData] = useState([]);

    React.useEffect(() => {
        setSoalData(JSON.parse(localStorage.getItem('data_soal_keterampilan')) || [])
    }, [])

    function clearSoal() {
        setSoalData([])
        localStorage.removeItem('data_soal_keterampilan')
    }
